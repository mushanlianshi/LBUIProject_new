...
- (void)setAttributeText:(NSAttributedString *)attributeText {
    ...
    self.labelCopy.attributedText = attributeText;
    [self stopScrollAnimation];
    [self setLabelsFrame];
}
...
- (void)setLabelAlignment:(NSTextAlignment)labelAlignment {
    ...
    self.label.textAlignment = _labelAlignment;
    [self setLabelsFrame];
}
...
- (void)setLabelsFrame {
    ...
    if (!self.autoBeginScroll) {
        ...
        NSLog(@"self 大小 width :%.f height: %.f",self.frame.size.width,self.frame.size.height);
